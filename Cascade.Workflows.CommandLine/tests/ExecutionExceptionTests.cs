...
internal sealed class ExecutionExceptionTests
{
    ...
    public void ExecutionExceptionTests_Throws()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(_logger.Logs.Count(x => x.LogLevel == LogLevel.Error), Is.EqualTo(1), "Error count");
            Assert.That(_logger.Logs.Count(x => x.LogLevel == LogLevel.Critical), Is.EqualTo(0), "Critical count");
        });
    }
}
